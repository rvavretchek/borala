...
- **Rolê:** um encontro específico criado por um organizador, com um conjunto de participantes, um raio, um modo de decisão e uma janela (horário de início + duração estimada).
- **Janela do rolê:** intervalo entre o horário de início e o fim estimado (início + duração). Usada para garantir que os bares candidatos estarão abertos durante todo o encontro.
...
| Ator | Papel |
|---|---|
| **Organizador** | Cria o rolê, convida, define raio, janela (horário + duração) e modo de decisão. |
...
- **RF32** — Organizador informa o **horário de início** e a **estimativa de duração** do rolê, definindo a janela usada para filtrar bares fechados no período.

> Numeração estável: requisitos adicionados após a primeira versão recebem o próximo número livre e são posicionados na seção temática, sem renumerar os existentes (preserva rastreabilidade).
...
- **RF18** — Exibir, para cada local: nome, foto, avaliação, faixa de preço, distância do ponto médio e situação de funcionamento **durante a janela do rolê** (aberto o tempo todo / fecha durante o rolê / fechado).
...
- **RF33** — Considerar como candidatos ao sorteio/eleição apenas os bares **abertos durante toda a janela do rolê**; bares que fecham antes do fim são sinalizados e excluídos do conjunto sorteável (ver RN08).
...
- **RNF02 — Custo de API:** arquitetura limitada a, no pior caso, ~1 chamada paga por rolê (a busca de places), e somente em cache miss. Cache de geocodificação (endereço fixo) e de resultados de places por região (geohash), com expiração por tempo. A filtragem pela janela do rolê é feita localmente sobre o cache, sem chamadas adicionais.
...
2. Define o raio (ou aceita o default), o horário de início, a duração estimada e o modo de decisão.
...
1. O sistema apresenta os bares dentro do raio, marcando como candidatos elegíveis apenas os abertos durante toda a janela do rolê.
...
**Must have**
- RF01, RF04, RF05, RF07, RF08, RF10, RF32 (conta, grupo, criação, convite e janela do rolê)
- RF11, RF12, RF13, RF14 (localização)
- RF15, RF16 (ponto médio)
- RF17, RF18, RF33 (lista de bares e elegibilidade por janela)
...
- **RN03** — Auto-expansão: se a busca devolver menos que o limiar (ex.: 5 locais) **elegíveis** — ou seja, abertos durante a janela do rolê — ampliar o raio em passos até um teto e avisar.
...
- **RN08** — Um bar só é candidato ao sorteio/eleição se estiver aberto durante **todo** o intervalo da janela do rolê (início até início + duração). Bares fechados ou que encerram antes do fim são exibidos como informação, mas não entram no conjunto sorteável/votável.
...
- **Atenção a campos e custo:** em qualquer provedor, os campos que o produto precisa (avaliação, faixa de preço, foto e — agora obrigatório — horário de funcionamento) são os mais caros. O custo é função dos campos pedidos, não só do número de chamadas — o que reforça o cache e a parcimônia de campos.
- **Filtragem pela janela do rolê é local:** o horário de funcionamento do bar (campo "Contact data" no Google, já previsto como SKU pago) vem na busca e é cacheado junto com o resto. A verificação de se o bar cobre a janela [início, início + duração] é lógica local sobre o cache — **não gera chamada extra**. Como horário de funcionamento é estável, isso cacheia bem; o que muda entre rolês é apenas a janela, comparada localmente. Reforça a escolha do Google, cujo dado de horário é confiável.
